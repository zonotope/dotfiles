...
Set [todo -> next (started | waiting) -> (done | cancelled)] as the todo state
sequence. Also log when I mark tasks started and done, log with notes whenever
they're marked blocked or canceled.
#+BEGIN_SRC emacs-lisp
  (setq org-todo-keywords '((sequence "TODO(t)" "NEXT(n)" "STARTED(s)" "WAITING(w@/!)"
                                      "|"
                                      "DONE(d!)" "CANCELED(c@)")))
#+END_SRC
...
Set 3 task priorities: A, B, and C.
#+BEGIN_SRC emacs-lisp
...
Set up org-pomodoro to use the pomodoro technique to track tasks. The default
shows pomodoro notifications in the echo area and messages buffer, but I set it
to show using the systemwide libnotify process to make them more noticeable.
#+BEGIN_SRC emacs-lisp
...
I also use [[https://github.com/nanjj/go-mod.el][go-mod.el]] to handle ~go.mod~ and ~go.sum~ files. I've loaded the
symlinked raw file for now until there's an official package in one of the
repositories for it.
#+BEGIN_SRC emacs-lisp
  (require 'go-mod-mode)
#+END_SRC
...
***** Language Server
~lsp-mode~ has a built in [[https://github.com/golang/go/wiki/gopls#emacs][gopls]] client out of the box.
#+BEGIN_SRC emacs-lisp
...
~lsp-ui~ gives nicer overlays
#+BEGIN_SRC emacs-lisp
   (use-package lsp-ui
     :commands lsp-ui-mode)
#+END_SRC

And ~company-lsp~ integrates company-mode completions with lsp-mode.
#+BEGIN_SRC emacs-lisp
...
We also want to install web-mode to edit tsx files.
#+BEGIN_SRC emacs-lisp
  (use-package web-mode
    :mode (("\\.tsx\\'" . web-mode)))
#+END_SRC

Now, let's install tide, enable it in typescript files, run the formatter before
every save, and enable web mode for tsx files
#+BEGIN_SRC emacs-lisp
  (use-package tide
