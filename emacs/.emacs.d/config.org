...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package avy
    ...
    :config (progn
     ...
     :config (set-face-attribute 'avy-lead-face nil :foreground red
                                                    :background nil))
...
**** Exporters
I use Org to manage my blog (of course). ~ox-hugo~ exports org to [[https://gohugo.io/][Hugo]] flavored
markdown and enables a nice workflow using only one file. ~ox-hugo-auto-export~
takes care of keeping the exported site in sync with each file save.
